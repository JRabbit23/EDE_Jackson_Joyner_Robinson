...
Capacity_by_Party_COR <- cor(Capacity_by_Party)
corrplot(Capacity_by_Party_COR,method="ellipse")




#ANOVA Test for party correlative effect on nameplate capacity
#ANOVA_Capacity <- aov(data = GOV_eGRID_10_20, NAMEPCAP ~ party)
#summary(ANOVA_Capacity)

#Test for correlative coefficient
#cor(GOV_eGRID_10_20$party,GOV_eGRID_10_20$NAMEPCAP,method="pearson")
...
summary(GOV_eGRID_10_20$NAMEPCAP)

#Make into time series object for nameplate cap

GOV_eGRID_10_20_aggregate <- GOV_eGRID_10_20 %>%
  group_by(YEAR) %>%
  summarize(NAMEPCAPSUM = sum(NAMEPCAP))
...
```{r Spatial Analysis, include=FALSE}

#Exploratory
mapview(States_shapefile_shp)

mapview(GOV_eGRID_10_20)
...
party_colors <- c(" " = "white",
                  "Democratic" = "blue",  # Assigning blue to Democratic
                  "Independent" = "gray",
                  "New Progressive Party (Puerto Rico)" = "black",
                  "Popular Democratic Party" = "yellow",
